...
stediGraphJoinStreamingDF = emailAndBirthYearStreamingDF.join(stediEventsStreamingView, expr("customer=email"))
...
stediGraphJoinStreamingDF.selectExpr("cast(customer as string) as key", 
"to_json(struct(*)) as value")\
  .writeStream \
  .format("kafka") \
  .option("kafka.bootstrap.servers", "localhost:9092") \
  .option("topic", "risk-assessment") \
